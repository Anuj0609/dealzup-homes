...
function Navbar() {
  ...
  const navItems = ["Home", "Buy", "Rent", "Sell", "About Us", "Contact Us"];
      ...
      <div className="px-2 lg:px-6 xl:px-[53px] flex justify-between py-6 xl:py-9 items-center">
        ...
        <div className="hidden lg:flex flex-row gap-3 xl:gap-4">
          {navItems.map((item, index) => (
            <div key={index} className="px-2 xl:px-4 cursor-pointer">
        ...
        <div className="hidden lg:flex bg-[#1E3A8A] items-center justify-center px-4 xl:px-5 py-2 lg:py-[10px] rounded-3xl cursor-pointer gap-2">
          <div className="font-poppins font-normal text-sm xl:text-[18px] text-center text-white">
            Login / Register
          </div>
      ...
      {isOpen && (
        ...
        >
          {navItems.map((item, index) => (
            <div
              key={index}
              className="py-2 text-lg font-poppins font-medium text-[#1E1E1E] hover:text-[#1E3A8A] cursor-pointer transition hover:scale-105 w-full text-center"
            >
              {item}
            </div>
          ))}
