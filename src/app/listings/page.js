...
export default function DreamHome() {
  ...
  const getUniqueOptions = (items, key) => {
    const uniqueSet = new Set(
      items
        .map((item) => {
          if (key === "type") return item.type || ""; // change if you have a real `type` field
          if (key === "propertyType") return item.propertyType || "";
          return item[key] || "";
        })
        .filter(Boolean)
    );
    return Array.from(uniqueSet).map((val) => ({ value: val, label: val }));
  };
  const typeOptions = getUniqueOptions(listings, "type");
  const propertyTypeOptions = getUniqueOptions(listings, "propertyType");
  const locationOptions = getUniqueOptions(listings, "city");
  ...
  return (
    <div
      className="h-[500px] bg-cover bg-center md:rounded-3xl xl:mx-[53px] md:my-10 py-10 mx-4 rounded-2xl"
        ...
        >
          <div className="flex flex-row gap-2 items-center w-full">
            ...
            <input
              ...
              className="w-full placeholder:font-poppins placeholder:font-normal placeholder:text-[20px] placeholder:text-[#979797] focus:outline-none"
            />
          </div>
